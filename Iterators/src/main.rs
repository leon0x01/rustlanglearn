...
fn main() {
     ...
     let x = a.iter().filter(|&x| *x >= 5).collect::<Vec<&u32>>();
     println!("the value are {:?}", x);

     let b = a.clone();
     let x = a.into_iter().filter(|x| *x >= 5).collect::<Vec<u32>>();
     println!("the value are {:?}", x);

     let mapped_value = b.iter().map(|x|2* *x).filter(|x| *x > 10).collect::<Vec<u32>>();
